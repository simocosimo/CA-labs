...
Reset_Handler   PROC
				...
				MOV		R0, #8	;x value
				SVC		0x01	;change to change func
				POP		{R2}
				STR		R2, [R1]
...
SVC_Handler     PROC
				...
				STMFD 	SP!, {R0-R12, LR}
				MRS		R1, psp
				LDR 	R2, [R1, #24]	;get the PC from the stack
				LDR 	R2, [R2,#-4]	;get the SVC instruction (PC was already +4ed when this launched)
				BIC 	R2, #0xFF000000 ;get the value of the SVC
				LSR 	R2, #16			;shift it to be nice and clean
